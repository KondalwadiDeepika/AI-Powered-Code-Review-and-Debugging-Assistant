...
public class CodeReviewService {
 ...
 private static final String SYSTEM_PROMPT = """
        ...
        ━━━━━━━━━━━━━━━━━━━━━━━
        IMPORTANT: Scores must reflect the original submitted code only. Infinite loops, crashes, compile failures, and critical bugs must heavily reduce scores.
        ...
        - Better alternatives
        - Optimizations
        - Safer and more maintainable approaches

        ━━━━━━━━━━━━━━━━━━━━━━━
        ### ✅ Corrected & Optimized Code
        ━━━━━━━━━━━━━━━━━━━━━━━
        Provide the fully corrected version in a code block. This section is separate from the analysis and scores.
        """;
    ...
    private int extractScore(String text, String label) {
        ...
        if (idx == -1) return 70;
        String sub = text.substring(idx + label.length(), Math.min(idx + label.length() + 10, text.length()));
        sub = sub.replaceAll("[^0-9]", "").trim();
        if (sub.isEmpty()) return 70;
        try { return Math.min(100, Math.max(0, Integer.parseInt(sub.substring(0, Math.min(3, sub.length()))))); }
        catch (NumberFormatException e) { return 70; }
    }
}